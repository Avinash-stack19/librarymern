...
const EbookManagement = () => {
  ...
  useEffect(() => {
    const fetchEbooks = async () => {
      try {
        const res = await axios.get("/api/user/ebooks", {
  ...
  const addEbook = async () => {
    try {
      await axios.post("/api/librarian/ebooks", newEbook, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(
        `/api/librarian/ebooks/${selectedEbook._id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const updateEbook = async () => {
    try {
      const res = await axios.put(
        `/api/librarian/ebooks/${editEbook._id}`,
        editEbook,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
